...
# Initialize the GAN model and move them to the device (GPU if available)
generator = Generator(latent_dim, vocab_size, hidden_dim, num_layers).to(device)
discriminator = Discriminator(vocab_size, hidden_dim, num_layers).to(device)
...
for epoch in range(num_epochs):
    for batch, lengths in data_loader:  # Unpack the batch and lengths
        real_sequences = batch[0].to(device)
        ...
        latent_vector = torch.randn(batch_size, 1, latent_dim).to(device)
        fake_sequences = generator(latent_vector)
